...
void CARTView::addOrder(int index, int textId, uint32_t sumCount, uint32_t sumPrice)
{
    ...
    //Unicode::snprintf(priceBuffer, BUFFER_SIZE, "%d", sumPrice);
...
{
    char tempBuffer[20];
    snprintf(tempBuffer, sizeof(tempBuffer), "%lu", (unsigned long)value); // 숫자 -> 문자열

    int len = strlen(tempBuffer);
    int commaCount = (len - 1) / 3; // 삽입할 콤마 개수
...
uint32_t CARTView::calculateTotalPrice()
{
    ...
    for (int i = 0; i < presenter->getCartSize(); i++)
    {
        totalPrice += presenter->getCartSumPrice(i); // 각 상품의 금액을 합산
    }
